...
{
    public class TestIndicatorWeightedMovingAverage
    {
        private readonly WeightedMovingAverage _sut;
        private const int Period = 14;

        /// <summary>
        /// Initialize test
        /// </summary>
        public TestIndicatorWeightedMovingAverage()
        {
            DataStream stream = new OHLCBarStream(new ForexSecurity("EURUSD"), BarInterval.FiveMin);
            _sut = new Quantler.Indicators.WeightedMovingAverage(Period, stream.DefaultInterval, stream);
        }
