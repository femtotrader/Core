...
{
    public class TestIndicatorExponentialMovingAverage
    {
        private readonly ExponentialMovingAverage _sut;
        private const int Period = 14;

        /// <summary>
        /// Initialize test
        /// </summary>
        public TestIndicatorExponentialMovingAverage()
        {
            DataStream stream = new OHLCBarStream(new ForexSecurity("EURUSD"), BarInterval.FiveMin);
            _sut = new ExponentialMovingAverage(Period, stream.DefaultInterval, stream);
        }
        ...
        {
            Util.TestIndicator(_sut, "ema", new[] { "Result" }, (indicator, results) =>
            {
